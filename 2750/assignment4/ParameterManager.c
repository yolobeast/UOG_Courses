...
int PM_parseFrom(ParameterManager *p, FILE *fp, char comment)
{
  ...
  while((returnfile=fgetc(fp))!=EOF )
  {
	...
	else
	{
      ...
      {
	if(!chekintype(fileinfo[a],setvalue))
	{
	  return(0);
	}
      }
      ...
      {
	if(!checkfloat(fileinfo[a],setvalue))
	{
	  return(0);
	}
      }
      ...
      {
	if(!chekbooltype(fileinfo[a],setvalue))
	{
	  return(0);
	}
      }
      ...
      {
	if(!chekstring(fileinfo[a],setvalue))
	{
	   return(0);
	}
	memset(fileinfo,0,sizeof fileinfo);
      ...
      {
	if(!checklist(fileinfo[a],setvalue))
	{
	  return(0);
	}
	memset(fileinfo,0,sizeof fileinfo);
