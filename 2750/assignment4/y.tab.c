...
{
       0,    44,    44,    45,    47,    48,    49,    50,    52,    62,
      73,    84,    89,    90,    92,   104,   116
};
#endif
...
{
  "$end", "error", "$undefined", "EQUALS", "SIMCOLONE", "QUOTES",
  "L_OPEN", "L_CLOSE", "COMMA", "WORD", "TITLE", "FIELDS", "BUTTONS",
  "$accept", "stmnt", "string", "assign", "expr", "listexpr", "listitem", YY_NULLPTR
};
#endif
...
{
      -8,    11,    15,    16,    17,     0,    -9,    18,    18,    19,
      19,    -9,    -9,    -4,    -9,    -9,     8,    -9,    -9,    -9,
      -9,    -9,    -9,    21,    -4,    -9,    20,    23,    24,    -9,
      -9,     9,    -9,     8,    -9
};
...
{
       0,     0,     0,     0,     0,     0,     3,     0,     0,     0,
       0,     1,     2,     0,    11,     8,     0,     9,    10,     4,
       5,     6,     7,     0,     0,    16,     0,     0,     0,    13,
      12,     0,    15,     0,    14
};
...
{
      11,     1,     2,     3,     4,    19,    20,    21,    22,     1,
       2,     3,     4,    24,     7,    25,    32,    33,     8,     9,
      10,    28,    15,    13,    29,    16,    27,    30,    34,    31,
      12,     0,    18
};

static const yytype_int8 yycheck[] =
{
       0,     9,    10,    11,    12,     9,    10,    11,    12,     9,
      10,    11,    12,     5,     3,     7,     7,     8,     3,     3,
       3,    24,     8,     5,     4,     6,     5,     4,    33,     5,
       5,    -1,    10
};
...
{
       0,     9,    10,    11,    12,    14,    16,     3,     3,     3,
       3,     0,    16,     5,    17,    17,     6,    18,    18,     9,
      10,    11,    12,    15,     5,     7,    19,     5,    15,     4,
       4,     5,     7,     8,    19
};
...
#line 1289 "y.tab.c" /* yacc.c:1646  */
  ...
  case 11:
#line 84 "yadc.y" /* yacc.c:1646  */
    {
				    setTable(tablevalues,(yyvsp[-2].sval),(yyvsp[0].sval));
			   }
#line 1297 "y.tab.c" /* yacc.c:1646  */
    break;

  case 12:
#line 89 "yadc.y" /* yacc.c:1646  */
    { (yyval.sval) = (yyvsp[-2].sval);}
#line 1303 "y.tab.c" /* yacc.c:1646  */
    break;

  case 14:
#line 92 "yadc.y" /* yacc.c:1646  */
    { if(buff==NULL)
				    {
				      buff=creatList(); 
				    }
				    strcpy(buff->b[buff->temp],(yyvsp[-3].sval));
				   // buff->listthings++;
				    buff->index++;
				    buff->temp++;
...
#line 1319 "y.tab.c" /* yacc.c:1646  */
  ...
  case 15:
#line 104 "yadc.y" /* yacc.c:1646  */
    {  if(buff==NULL)
				    {
				      buff=creatList(); 
				    }
				    strcpy(buff->b[buff->temp],(yyvsp[-2].sval));
				    //buff->listthings++;
				    buff->index++;
				    buff->temp++;
				    //list_add_element(buff,$1);
...
int main(int arc,char ** argv)
{
	...
	{
	  sprintf(file, "%s/%s.java", folder,tableGet(tablevalues,buttons->b[i]));
	  outfille = fopen(file, "w");
	  
	  fprintf(outfille,"import java.awt.event.*;\n\n");
	  fprintf(outfille,"public class %s implements ActionListener\n",tableGet(tablevalues,buttons->b[i]));
	  fprintf(outfille,"{\n");
	  fprintf(outfille,"\t%sFieldEdit dialog;\n",nameoffile);
	  fprintf(outfille,"\tpublic %s(%sFieldEdit dialog)",tableGet(tablevalues,buttons->b[i]),nameoffile);
	  fprintf(outfille,"\t{\n");
	  fprintf(outfille,"\t\tthis.dialog = dialog;");
	  fprintf(outfille,"\t}\n");
	  fprintf(outfille,"\tpublic void actionPerformed(ActionEvent e)");
	  fprintf(outfille,"\t{\n");
	  if(strcmp(buttons->b[i],"ADD")==0)
	  {
	    fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"Adding to the record\");\n");
	  }
	  else if(strcmp(buttons->b[i],"REMOVE")==0)
	  {
	    fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"Removeing to the record\");\n");
	  }
	  else if(strcmp(buttons->b[i],"QUERY")==0)
	  {
	    fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"Querying to the record\");\n");
	  }
	  else if(strcmp(buttons->b[i],"UPDATE")==0)
	  {
	    fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"Updateing to the record\");\n");
	  }
	  else if(strcmp(buttons->b[i],"Get")==0)
	  {
	    for(x=0;x<feilds->temp;x++)
	    {
		fprintf(outfille,"\t\ttry\n");
		fprintf(outfille,"\t\t{\n");
		fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"%s =\" + dialog.getDC%s());\n",feilds->b[x],feilds->b[x]);
		fprintf(outfille,"\t\t}\n");
		fprintf(outfille,"\t\tcatch(Exception tr)\n");
		fprintf(outfille,"\t\t{\n");
		fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"%s = wrong type\");\n",feilds->b[x]);
		fprintf(outfille,"\t\t}\n");
	    }
	  } 
	  else if(strcmp(buttons->b[i],"Set")==0)
	  {
	    for(x=0;x<feilds->temp;x++)
	    {
	      if(strcmp(tableGet(tablevalues,feilds->b[i]),"integer")==0)
	      {		
		fprintf(outfille,"\t\ttry\n");
		fprintf(outfille,"\t\t{\n");
		fprintf(outfille,"\t\t\tdialog.setDC%s(\"123\");\n",feilds->b[x],feilds->b[x]);
		fprintf(outfille,"\t\t}\n");
		fprintf(outfille,"\t\tcatch(Exception tr)\n");
		fprintf(outfille,"\t\t{\n");
		fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"%s = wrong type\");\n",feilds->b[x]);
		fprintf(outfille,"\t\t}\n");
	      ...
	      else if(strcmp(tableGet(tablevalues,feilds->b[i]),"float")==0)
	      {		
		fprintf(outfille,"\t\ttry\n");
		fprintf(outfille,"\t\t{\n");
		fprintf(outfille,"\t\t\tdialog.setDC%s(\"123.00\");\n",feilds->b[x],feilds->b[x]);
		fprintf(outfille,"\t\t}\n");
		fprintf(outfille,"\t\tcatch(Exception tr)\n");
		fprintf(outfille,"\t\t{\n");
		fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"%s = wrong type\");\n",feilds->b[x]);
		fprintf(outfille,"\t\t}\n");
	      ...
	      else if(strcmp(tableGet(tablevalues,feilds->b[i]),"string")==0)
	      {		
		fprintf(outfille,"\t\ttry\n");
		fprintf(outfille,"\t\t{\n");
		fprintf(outfille,"\t\t\tdialog.setDC%s(\"123\");\n",feilds->b[x],feilds->b[x]);
	  ...
	  {
	     fprintf(outfille,"\t\t\tdialog.appendToStatusArea (\"Click!!\");\n");
	  }
	  fprintf(outfille,"\t}\n");
	  fprintf(outfille,"}\n");
	  fclose(outfille);
